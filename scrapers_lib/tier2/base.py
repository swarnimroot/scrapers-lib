...
def warmed_curl_session(
    ...
    *,
    ...
    (Akamai HTTP/2 RST-stream gate workaround). When ``warm=True``, fires a
    homepage GET and sleeps ``warm_delay`` seconds before yielding so cookies
    settle. Warm-up failures are swallowed and logged at DEBUG.

    Used by Tier 2 (HP, MSI) and Tier 3 (BestBuy) to clear Akamai/CDN gates
    that reject plain httpx.
    """
