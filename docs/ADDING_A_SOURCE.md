...
|---|---|---|---|---|---|
| **Dell** | Shop page carries per-tile `data-url` attributes pointing at `csbapi/unifiedpd/techspecs/{lang}/{country}/{segment}/{customerset}/{sku}`. Call that endpoint through the same stealth Playwright session that loaded the shop page so Akamai cookies ride along. **Wave 2h add-on (v1.5.0):** `include_options=True` derives a sibling `cty/pdp` configurator URL from the shop URL's `spd-slug` + the first tile's order code, navigates to it on the same stealth session, and parses the 10 SSR hardware modules (`div.accordion-box.single-column-accordion` → `h2.module-title` + `div.option.detailed-option[data-option-id][data-status]`) into the new `ProductSnapshot.options` field. Status vocabulary: `selected` / `available` / `unavailable`. Software / accessories modules (Microsoft 365, antivirus) live in a different `<input type="radio">` DOM and are intentionally not surfaced. | `www.dell.com/csbapi/unifiedpd/techspecs/...` (primary, per tile); `www.dell.com/<locale>/shop/cty/pdp/spd/<spd-slug>/<oc>` (configurator option menu, opt-in via `include_options=True`). | Akamai. Needs Playwright + `playwright-stealth` + homepage warming. | Per-SKU (each pre-built BTO tile). Configurator option menu is product-line-level, not per-tile. | **Verified** (Wave 2a, on Alienware Aurora 16X + Dell XPS 16; Wave 2h configurator on Alienware Aurora 16 `useac16250hbtshtgb`). |
