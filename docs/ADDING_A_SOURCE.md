...
| **Lenovo** | Likely PSREF — a dedicated spec-reference site at `psref.lenovo.com/l/Product/<line>/<model>?tab=spec`. Probably plain `httpx`. | `psref.lenovo.com` | Probably none. | Per-model family (not per-SKU variant). | **Unverified.** Recon required before coding. |
| HP, ASUS, Acer, MSI | Unknown. | — | — | — | **Recon required.** |
