...
| **HP (shop PDP)** | Shop PDP HTML embeds the full page state as a JSON-encoded HTML comment inside `<div id="data" style="display:none"><!-- {...} --></div>`. Walk that state to `slugInfo.components.pdpCTOConfiguration.configurations` — a list of pre-built "Recommended Configuration" tiles. Each tile carries `configCatentryId` (HP's per-tile SKU, analog of Dell's `data-oc`) and `fullSpecs.technical_specifications` — a list of `{name, value: [{value, subheading: "Included in Current Configuration" \| "Alternate Options"}]}`. Multi-option values use `<br />` separators within one value field. Emit one snapshot per tile. **Coverage caveat:** `technical_specifications` is the config-picker summary (~12 categories); the "Tech Specs" UI section (Dimensions, Weight, Ports, Power, Audio, Sensors, Warranty — ~20 more) is hydrated client-side and NOT in the httpx response. HP's bot gate rejects vanilla AND stealth Playwright on `/shop/pdp/` URLs with `ERR_HTTP2_PROTOCOL_ERROR` even with homepage warming — full-coverage alternative is HP's QuickSpecs PDFs at `h20195.www2.hp.com`, feasible in a future wave once a PDP-SKU → doc-ID lookup is built. | `www.hp.com/us-en/shop/pdp/<slug>` (the PDP URL itself; state JSON is in-page) | None for plain httpx; very aggressive anti-bot against browsers. | Per-tile (each `configCatentryId`), same as Dell. | **Verified** (Wave 2b, on Omen Max 16t-ah000 + Pavilion 16z-ag000). |
| **ASUS (ROG spec)** | Marketing/spec surface `rog.asus.com/laptops/<line>/<model>/spec/` is plain-httpx-reachable with ~1 MB of SSR'd HTML containing 20+ `<h2>`-headed spec sections (richest Tier 2 coverage so far including Dimensions / Weight / I/O Ports / Power Supply / Security / Expansion Slots / Wireless version — everything HP hides). Each h2 has class `ProductSpec__productSpecItemTitle__<hash>` (CSS module); its next-sibling `<div>` holds per-SKU variant rows with class `ProductSpec__rowItem__<hash>`. **The CSS-module hash suffixes change on every build** — match with `classname.startswith("ProductSpec__productSpecItemTitle__")` rather than exact token. Per-spec values dedupe first-occurrence (ASUS emits one row per SKU permutation; identical rows collapse). One snapshot per URL; multi-SKU alternatives embedded inline. Note that `shop.asus.com` IS the commerce surface for actual prices, but it is **DataDome-gated** and off-limits without paid anti-bot bypass; ROG spec page therefore emits no prices (like Lenovo PSREF). Brand normalized to "ASUS" (JSON-LD says "ROG", a sub-brand); raw.jsonld_brand preserves the original. | `rog.asus.com/laptops/<line>/<model>/spec/` (also works without trailing `/spec/`) | None on the ROG spec surface. `shop.asus.com` has DataDome. | Per-model family (one snapshot per URL; alternatives in spec values). | **Verified** (Wave 2b, on ROG Strix G16 2025 + ROG Zephyrus G16 2026). |
| Acer, MSI | **Deferred (post-demo).** Not required for Demo 2 max-spec comparison. Revisit after the demo ships if broader coverage is needed. | — | — | — | **Deferred.** |
