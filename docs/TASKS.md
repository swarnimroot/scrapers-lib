...
- **Latest tag:** **v1.1.0** (commit 22c7e85). Post-tag polish landed at cf6e58f. Wave 2e step 1 (HP recon) committed at 1a79fe7. **Wave 2e steps 2 + 4 shipped this session** (HP build + ASUS www) — staged for commit. Wave 2e step 3 (Lenovo URL recon) resolved 2026-05-07 as **NO** (consumer PDP carries no PSREF link; document the constraint, defer the search-bridge resolver).
...
- **Next direction:** Wave 2e closure (tag v1.2.0; commit + version bump pending), then **Wave 2f — Tier 2 expansion: Acer + MSI greenfield fetchers** (recon required first per `docs/ADDING_A_SOURCE.md`). Driver: Demo 2's consumer-side data-model target — the **`Competitor Columns` 83-row spec schema** (CPU / GPU / Memory / Display / Battery / I/O / Thermals / Design) — needs ~80–90% raw coverage across all six manufacturer brands. Library remains **frozen at v1.x** public API; Wave 2e/2f changes are additive (new fetcher modules + behind-fetcher acquisition rewrite). Pilot 1 brainstorming (pivoted Demo 1 per `project_demo1_pulse_check`) and BestBuy Developer API activation (per `project_bestbuy_api_dormant`) remain on hold.
